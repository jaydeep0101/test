...
for (let canvas of canvases) {
    ...
    function buildCanvas(config,  waves, Wave, h, animate ) {

        
    
        const waveColors = [ '#418dcb1a', '#418dcb80', '#418dcb4d' ];
        const waveColorsDark = [ '#79bdfebf', '#79bdfe1a', '#79bdfe80' ];
        ...
        const buildSystem = () => {
            ...
            for (let i = 0; i < config.waveCount; i++) {
                let mod = {
                    ...
                    color: currentMode === 'dark' ? waveColorsDark[i] : waveColors[i]
                };
    ...
    $('#mode').click(function(){
        waves = [];
        buildCanvas( config, waves, Wave, h , animate );
    });
